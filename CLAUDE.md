...
- Gli **scatti studio dei ricambi** sono generati con Pillow (`knockout_light_bg` per i pezzi in acciaio chiaro, `knockout_dark_part` per quelli scuri come coltello/zappa, così i fori mostrano il fondo) a partire dai file sorgente `assets/sources/source-prod-*.png` (uno per pezzo: mazze/vomeri/denti/ricambi/coltelli/zappe); i sorgenti sono in `.gitignore` (pesanti, non deployare).
